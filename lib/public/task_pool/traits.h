...
template <class Allocator> class task_pool_t;

// template< template< typename, typename... > class Allocator, typename Value,
// typename... Ts > class task_pool_t< Allocator< Value, Ts... > >;
...
template <typename T, typename = void> struct is_duration : std::false_type {};

template <typename T>
struct is_duration<T, be_void_t<typename T::rep, typename T::period>>
    : std::true_type {};
...
template <typename R, typename... Args> struct arguments<R(Args...)> {
...
template <typename T, typename = void> struct allocator_value {
  using type = void;
};
...
template <
    std::size_t N, typename TupleArgs,
...
template <
    ...
    std::enable_if_t<std::greater<std::size_t>{}(
                         std::tuple_size<std::decay_t<TupleArgs>>::value, N),
                     bool> = true>
auto nth_allocator_value(TupleArgs &) -> typename allocator_value<
    std::tuple_element_t<N, std::decay_t<TupleArgs>>>::type;

template <typename T, class Functor = std::decay_t<T> >
struct wants_allocator
    : public wants_allocator<decltype(&Functor::operator())> {};
...
template <typename R, typename... Args>
struct wants_allocator<R (*)(Args...)>
    : public contains_allocator_arg<Args...> {
  using value_type =
      decltype(nth_allocator_value<1>(std::declval<std::tuple<Args...> &>()));
};

template <class C, typename R, typename... Args>
struct wants_allocator<R (C::*)(Args...)>
    : public contains_allocator_arg<Args...> {
  using value_type =
      decltype(nth_allocator_value<1>(std::declval<std::tuple<Args...> &>()));
};
...
template <class C, typename R, typename... Args>
struct wants_allocator<R (C::*)(Args...) const>
    : public contains_allocator_arg<Args...> {
  using value_type =
      decltype(nth_allocator_value<1>(std::declval<std::tuple<Args...> &>()));
};
...
struct is_allocator_constructible<
    T,
...
template <typename T>
struct is_future_status<T, be_void_t<decltype(T::ready), decltype(T::timeout),
                                     decltype(T::deferred)>> : std::is_enum<T> {
};
...
template <typename Promise>
using set_value_t =
    decltype(&Promise::set_value); // not the best option but only one i could
                                   // get working

template <typename Promise>
using set_exception_t = decltype(std::declval<Promise>().set_exception(
    std::declval<std::exception_ptr>()));

template <typename T, typename = void> struct is_supported : std::false_type {};

template <template <typename> class T, typename V>
struct is_supported<T<V>, be_void_t<get_future_t<T<V>>, set_value_t<T<V>>,
                                    set_exception_t<T<V>>>>
    : std::conditional<is_future<get_future_t<T<V>>>::value, std::true_type,
                       std::false_type> {};
...
template <typename T, std::enable_if_t<be::is_future<T>::value, bool> = true>
auto wrap_future_argument(T &&t) {
  using status_type = decltype(std::declval<T>().wait_for(
      std::declval<std::chrono::seconds>()));
...
template <typename Promise, typename Callable, typename Instance,
          typename Arguments, std::size_t... Is,
          std::enable_if_t<be_is_void_v<future_api::get_result_t<
                               promise_api::get_future_t<Promise>>> &&
                               std::is_object<Callable>::value,
                           bool> = true>
void invoke_deferred_task(Promise &promise, Callable callable, Instance *self,
                          Arguments &arguments,
...
template <typename Promise, typename Callable, typename Arguments,
          std::size_t... Is,
          std::enable_if_t<be_is_void_v<future_api::get_result_t<
                               promise_api::get_future_t<Promise>>> &&
                               is_function_pointer_v<Callable>,
                           bool> = true>
void invoke_deferred_task(Promise &promise, Callable callable,
                          Arguments &arguments,
                          std::index_sequence<Is...> /*Is*/) {
...
bool check_argument_status(Arguments &arguments,
                           std::index_sequence<Is...> /*Is*/) {
  std::array<bool, sizeof...(Is)> args_status{
      std::get<Is>(arguments).is_ready()...};
  return std::all_of(args_status.begin(), args_status.end(),
...
template <typename Pipe> using future_t = decltype(Pipe::future_);

} // namespace pipe_api

template <typename T, typename = void> struct is_pipe : std::false_type {};
