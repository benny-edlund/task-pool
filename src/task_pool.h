 ...
 *
 * @details Users may submit tasks that take a be::stop_token as its last argument and the task_pool
 * will provide a token for the function when executing that may be used to break out of long running
 * work.
 *
 * @code{.cpp}
 ...
 * @endcode
 *
 * stop_tokens are a means to abort running operations and as such do not fire when the task_pool is
 * trying to complete work using `task_pool::wait_for_tasks` however it does fire on destruction.
 *
 * @code{.cpp}
 * {
 *     task_pool pool;
 *     auto f = pool.submit( []( be::stop_token token ){ while( !token ) { std::this_thread::sleep_for(1ms); } );
 *     // pool.wait_for_tasks(); // <-- this would deadlock
 * }                             // <-- destruction fires the stop token allowing task to complete
 * @endcode
 *
 */
struct stop_token
...
class task_pool
{
    ...
    struct task_proxy
    {
        ...
        explicit task_proxy(std::allocator_arg_t /*x*/, Allocator<Task> const & /* user_allocator*/, Task *task)
    ...
    /**
     * @brief Creates a task from some callable as a new type with allocator support
     */
  ...
  public:
    ...
    /**
     * @brief Resets the task_pool to the given amount of threads (completes all tasks)
     */
    void reset(const unsigned = 0);

    /**
     * @brief Sets the stop token and waits for all tasks to complete. Pool must be reset before it can process
     * tasks again
     */
    void abort();
    ...
    /**
     * @brief Blocks calling thread until all tasks have completed. No tasked may be submitted while pool is waiting
     */
    void wait_for_tasks();
    ...
    std::future<R> submit(F &&task, A &&...args)
    {
        ...
        auto task_future = promise.get_future();
        push_task(
            make_task([task_function = std::bind(std::forward<F>(task), std::forward<A>(args)..., get_stop_token()),
                          task_promise = std::move(promise)]() mutable {
                try {
                    task_function();
                    task_promise.set_value();
    ...
    BE_NODISGARD std::future<R> submit(F &&task, Args &&...args)
    {
        ...
        auto future = promise.get_future();
        push_task(
            make_task([task_function = std::bind(std::forward<F>(task), std::forward<Args>(args)..., get_stop_token()),
                          task_promise = std::move(promise)]() mutable {
                try {
                    task_promise.set_value(task_function());
                } catch (...) {
    ...
    template<template<typename> class UserAllocator,
        ...
        typename T,
        std::enable_if_t<be_is_void_v<R>, bool> = true>
    BE_NODISGARD std::future<R> submit(std::allocator_arg_t x, UserAllocator<T> const &allocator, F &&task, A &&...args)
    {
    ...
    BE_NODISGARD std::future<R> submit(std::allocator_arg_t x, UserAllocator<T> const &allocator, F &&task, A &&...args)
    {
        ...
        push_task(make_task(x,
            allocator,
            [task_function = std::bind(std::forward<F>(task), std::forward<A>(args)..., get_stop_token()),
                task_promise = std::move(promise)]() mutable {
                try {
                    task_function();
                    task_promise.set_value();
        ...
        submit(std::allocator_arg_t x, UserAllocator<T> const &allocator, F &&task, Args &&...args)
    {
        ...
        push_task(make_task(x,
            allocator,
            [task_function = std::bind(std::forward<F>(task), std::forward<Args>(args)..., get_stop_token()),
                task_promise = std::move(promise)]() mutable {
                try {
                    task_promise.set_value(task_function());
                } catch (...) {
