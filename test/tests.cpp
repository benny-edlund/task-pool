...
TEST_CASE( "submit( f, future )", "[task_pool][submit]" )
{
    const int          expected = 42;
    be::task_pool      pool( 1 );
...
// TEST_CASE( "wrap_func", "[task_pool][submit]" )
// {
...
//     auto defered_fun = wrap_func( fun, std::move( future ) );
//     pool.wait_for_tasks();
//     REQUIRE( defered_fun() == expected );
// }