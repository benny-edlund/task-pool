 ...
 *
 * @details
 * First off dont write this program, its a bad one. Im intentially going with a
 * poor design to show retrying of tasks.
 *
 * The goal of this program is build an image collage of a configurable density
 * from Wikipedia random articles.
 *
 * The program solves this by using curl to query the random/summary article. It
 * then locates the article image, decompresses it and inserts it into the final
 * image after resizing it. The final result is contigously updated.
 *
 * The trouble is that the program can only deal with jpeg images and not all
 * article summaries use jpegs as their image. Further it seems that Wikipedia
 * routinely renames gifs and pngs to jpg so its quite likley that the jpeg
 * decompression fails. Experiments show about a 5% success rate.
 *
 * This deficiancy in the program was not resolved by adding support for other
 * image format. Instead we brute foce and just re-runs the task. Poor
 * wikipedia.
 *
 * Needless to say you should not do this...but it does let us deal with a lot
 * of errors.
 */
#include <CLI/CLI.hpp>
#include <algorithm>
...
namespace {
std::atomic_size_t s_total_queries{0};
std::atomic_size_t s_success_queries{0};
} // namespace

struct task_failure : public std::runtime_error {
  explicit task_failure(std::string const &msg) : std::runtime_error(msg) {}
};
...
public:
  ...
  explicit Image(dimentions dims) // NOLINT
      : dims_{dims}, pixels_(dims_.size(), pixel{0, 0, 0}) {}
  ~Image() = default;
  Image(Image const &) = delete;
  Image(Image &&other) noexcept
      : dims_(other.dims_), pixels_(std::move(other.pixels_)) {
    other.dims_ = {};
  }
  ...
  explicit operator bool() const noexcept {
    return pixels_.size() == dims_.size();
  }
};
...
auto compress(Image &img) // NOLINT
{
  if (img.dims().size() != 0) {
    ...
    std::vector<unsigned char> raw_image(img.dims().width * img.dims().height *
                                         3);
    std::size_t i = 0;
    ...
    auto ec = tjCompress2(jpegCompressor, raw_image.data(), width, 0, height,
                          TJPF_RGB, &compressedImage, &jpegSize, TJSAMP_444,
...
Image decompress(std::vector<std::uint8_t> data) {
  ...
  if (0 != tjDecompressHeader2(jpegDecompressor, data.data(),
                               data.size() * sizeof(std::uint8_t), &width,
                               &height, &subsample)) {
    tjDestroy(jpegDecompressor);
    throw task_failure(
        fmt::format("[ {} ]: Failed to decompress jpeg header  ( {} )",
                    __LINE__, tjGetErrorStr()));
  }
  Image img(dimentions{static_cast<std::size_t>(width),
                       static_cast<std::size_t>(height)});
  if (0 != tjDecompress2(jpegDecompressor, data.data(), data.size(),
                         &img.pixels().data()->red, width, width * 3, height,
                         TJPF_RGB, 0)) {
    tjDestroy(jpegDecompressor);
    throw task_failure(
        fmt::format("[ {} ]: Failed to decompress jpeg body  ( {} )", __LINE__,
...
unsigned char *write(std::pair<unsigned char *, long unsigned int> data,
  ...
  if (data.first != nullptr) {
    ...
    // NOLINTNEXTLINE
    writer.write(reinterpret_cast<char *>(data.first),
                 std::streamsize(data.second));
  }
  return data.first;
}
 ...
 * @return size_t
 */
static size_t curl_write_function_(void *contents, size_t size, size_t nmemb,
                                   void *userp) {
...
std::vector<std::uint8_t> curl(std::string url) {
  ...
  throw task_failure(
      fmt::format("[ {} ]: Failed to initialize curl", __LINE__));
}
...
Image resize_image(dimentions size, Image input, be::stop_token abort) {
  try {
    ...
    float scale_height = static_cast<float>(input.dims().height) /
    ...
    throw task_failure(fmt::format("[ {} ]: Failed to resize image ( {} )\n",
                                   __LINE__, e.what()));
  }
}
...
void blit_image(ImageSection output, Image input, be::stop_token abort) {
  ...
  try {
    while (!abort) {
      ...
      output.owner.pixels().at(index) =
          input.pixels().at(w + input.dims().width * h);
    ...
    throw task_failure(fmt::format(
        "[ {} ]: Failed to insert image section ( {} )\n", __LINE__, e.what()));
  }
}
/**
...
int main(int argc, char **argv) // NOLINT
{
  ...
  app.description(
      "Create a collage of random images X tiles wide and Y tiles high.");
  ...
  if (tiles_height > 0 && tiles_width > 0) {
    ...
    auto fill_tile = [&](std::size_t tile) -> std::future<void> {
      ...
      // clang-format off
            auto pipe = pool | &find_random_jpeg 
                             ...
                             | &decompress 
                             | resize;
            return pool.submit( std::launch::async, &blit_image, ImageSection{ output, start }, std::move(pipe) );
      // clang-format on
    };
    // our main compression and write function.
    auto write_file = [&]() -> std::future<void> {
      auto jpeg_data =
          pool.submit(std::launch::async, &compress, std::ref(output));
      auto jpeg_ptr = pool.submit(std::launch::async, &write,
                                  std::move(jpeg_data), std::ref(filename));
      return pool.submit(std::launch::async, &tjFree, std::move(jpeg_ptr));
    };
    ...
    auto render_ui = [&](std::vector<bool> const &status) {
      ...
      int row = 0;
      move(++row, 0);
      printw("Building image collage from random Wikipedia articles "
             "contigously writing to '%s'",
             filename.c_str());
      move(++row, 0);
      printw("(Esc to stop)");
      ++row;
    ...
    // If we know what tile we are working on we can safely generate a view into
    // the output image that is unique to each task so we can write concurrently
    // to it without locks. As tiles are completed we will reduce this vector.
    // This is done in the main thread only so will not need any locks.
    using section_vector =
        std::vector<std::pair<std::size_t, std::future<void>>>;
    ...
    for (;;) {
      ...
      // This is a efficient way to move all the jobs that are completed to the
      // end of the vector.
      auto new_end = std::partition(
          sections.begin(), sections.end(), [](auto const &tile) {
            return tile.second.wait_for(0s) != std::future_status::ready;
          });
      std::vector<std::size_t> relaunch;
      bool any_success = false;
      // Check the result of all tasks that have completed. Since we use
      // exceptions a failure is a thrown exception. If its a 'task_failure' we
      // will requeue the tile and try another random article
      ...
      // We are IO bound on the internet queries so no need to busy check this
      // but still we want to show we are responsive in the ui
    ...
    try {
      ...
      printw("%s",
             fmt::format("Failed to write image [{}]\n", e.what()).c_str());
      refresh();
    }
