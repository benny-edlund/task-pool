...
using string_view_type = std::experimental::string_view;
#else
#include <string_view>
...
/**
 * This example is based on https://github.com/OsasAzamegbe/http-server please
 * visit to view the original
 */

void log_message_(string_view_type filename, int fileline, std::string const &msg) {
...
#define LOGGER(i_fmt_file, i_fmt_line, ...)                                    \
  [](char const *i_fmt_filename, int i_fmt_fileline, auto &&i_fmt_format,      \
     auto &&...i_fmt_args) {                                                   \
    constexpr std::size_t log_fmt_args_count = sizeof...(i_fmt_args);          \
    if (log_fmt_args_count) {                                                  \
      log_message_(i_fmt_filename, i_fmt_fileline,                             \
                   fmt::format(i_fmt_format, i_fmt_args...));                  \
    } else {                                                                   \
...
SocketData receive_data(int socket) {
  ...
  ssize_t bytesReceived =
      read(socket, static_cast<void *>(buffer.data()), BUFFER_SIZE);
  if (bytesReceived < 0) {
    CONSOL_LOG(string_view_type("Error read from socket [{}]\n"), strerror(errno));
    return {};
  };

  return std::make_pair(
      socket, Data(std::make_move_iterator(buffer.begin()),
                   std::make_move_iterator(
                       buffer.begin() + sizeof(char) * static_cast<std::size_t>(
                                                           bytesReceived))));
}
...
void close_connection(int socket) {
  close(socket);
  CONSOL_LOG(string_view_type("Connection closed\n"));
}

tcp_server::tcp_server(std::string ip_address, unsigned short port)
    : m_ip_address(std::move(ip_address)), m_port(port), m_socket(),
...
tcp_server::~tcp_server() { close(m_socket); }
...
void tcp_server::shutdown() {
  CONSOL_LOG(string_view_type("Shutting down\n"));
  m_pool.abort();
}
...
bool tcp_server::start_server() {
  ...
  if (m_socket < 0) {
    CONSOL_LOG(string_view_type("Cannot create socket [{}]\n"), strerror(errno));
    return false;
  }
  // NOLINTNEXTLINE
  if (bind(m_socket, reinterpret_cast<sockaddr *>(&m_socketAddress),
           m_socketAddress_len) < 0) {
    CONSOL_LOG(string_view_type("Cannot bind server to port [{}]\n"),
               strerror(errno));
    return false;
  }
  CONSOL_LOG(string_view_type("Server running on {}:{}\n"), m_ip_address, m_port);
  return true;
}
...
int tcp_server::accept_connection() {
  ...
  if (listen(m_socket, s_max_connections) < 0) {
    CONSOL_LOG(string_view_type("Cannot listen on port [{}]\n"), strerror(errno));
    return 0;
  }
  auto socket = // NOLINTNEXTLINE
      accept(m_socket, reinterpret_cast<sockaddr *>(&m_socketAddress),
             &m_socketAddress_len);

  if (socket < 0) {
    CONSOL_LOG(
        string_view_type("Server failed to accept incoming connection [{}]\n"),
        strerror(errno));
    return 0;
  }
  return socket;
